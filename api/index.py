...
EDGE_CONFIG_CONN = os.environ.get("EDGE_CONFIG")            # read connection string: https://edge-config.vercel.com/<id>?token=<read_token>
VERCEL_ACCESS_TOKEN = os.environ.get("VERCEL_ACCESS_TOKEN") # for writes to REST API
VERCEL_TEAM_ID = os.environ.get("VERCEL_TEAM_ID")           # optional if team-scoped Edge Config
...
def _ensure_edge_ids():
    """Parse EDGE_CONFIG only when first needed, so app doesn't crash at import."""
    global EC_ID, EC_READ_TOKEN
    if EC_ID and EC_READ_TOKEN:
        return
    if not EDGE_CONFIG_CONN:
        raise RuntimeError("EDGE_CONFIG env var is missing (connection string with read token).")
    EC_ID, EC_READ_TOKEN = _parse_edge_config(EDGE_CONFIG_CONN)


def ec_read_item(key: str):
    """Fast reads using the Edge endpoint."""
    _ensure_edge_ids()
    import requests
...
def ec_patch_items(items: list[dict]) -> None:
    """Writes via REST API (requires VERCEL_ACCESS_TOKEN)."""
    _ensure_edge_ids()
    ...
    if r.status_code >= 400:
        raise RuntimeError(f"Edge Config write failed ({r.status_code}): {r.text}")


# ------------------------------------------------------------------------------
# Compact tournament document (fits Edge Config size limits)
# ------------------------------------------------------------------------------
...
# ------------------------------------------------------------------------------
# KTS tie-breakers in-memory graph
# ------------------------------------------------------------------------------
...
class Node:
    ...
    ties: List[object] = field(default_factory=list)    # (ties count as 'lost round' wrt DDD in your policy)
...
def compute_standings(t: dict):
    ...
    for p in nodes.values():
        aa = p.match_points()
        bbb = int(round(opp_win_pct(p), 3) * 1000)
        bbb = max(0, min(999, bbb))
        acc, nopp = 0.0, 0
        for opp in p.wins + p.losses:
            if opp == "BYE":
                continue
            acc += opp_win_pct(opp)
            nopp += 1
        ccc = int(round((acc / nopp) if nopp else 0.0, 3) * 1000)
        ccc = max(0, min(999, ccc))
...
# ------------------------------------------------------------------------------
# Pairing logic (Swiss-ish): minimize rematches, minimize point gaps, BYE -> lowest
# ------------------------------------------------------------------------------
...
@app.get("/api/health")
def health():
    """Quick read/write probe to Edge Config; returns any error details."""
    try:
        _ensure_edge_ids()
        key = "health_probe"
...
def api_pair_next(tid):
    try:
        ...
        # return pairs with names (no need to wait for propagation)
...
def api_finalize_round(tid):
    ...
    BYE matches are created with r='BYE' already.
    """
    try:
        data = request.get_json(force=True)
        ...
        ec_patch_items([{"operation": "upsert", "key": tid_key(tid), "value": t}])
        # compute standings from updated doc (no propagation delay)
